...
As you can see from our databse creation, there are different data types in SQL. In addition to the data type, you can also specify 
The data needs to be in the correct order and the correct format as designated by the variable type we assinged it when creating the table (i.e., ```AGENT_CODE``` was assigned the data)
...
### Exercise 2: How to change data in an already existing table
...
```{sql class.source = 'fold-hide', eval=F, warning=F, error=FALSE, eval=FALSE, warning=FALSE}
ALTER TABLE AGENTS
