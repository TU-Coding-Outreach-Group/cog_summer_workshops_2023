...
```{python, echo = FALSE, error=FALSE,eval=FALSE, warning=FALSE}
import pyodbc 
...
```{sql, Create Database, connection=con_r, echo = FALSE, error=FALSE,eval=FALSE, warning=FALSE}
CREATE DATABASE COG_SQL
```


```{sql, connection=con_r, echo = FALSE, error=FALSE, warning=FALSE}
USE COG_SQL
```
...
```{sql, connection=con_r, echo = FALSE, error=FALSE,eval=FALSE, warning=FALSE}
CREATE TABLE  AGENTS (	
...
Semi-colons are used to end commands, as you can also see above. When you have a multi-line command you would like to execute, it can either be broken up into those separate lines (like you can see above, when we were setting up the columns for our tables), but they can also be all on one line if you would like.
...
### How to show only certain data from our tables

For this exercise, we want you to use your cheat sheet to figure out how to only display the cities customers are in, for our ORDERS table. We will give you the general syntax outline, but put your guess at which functions we would use in the chat!
...
```{sql class.source = 'fold-hide', connection=con_r, eval=F, warning=F, echo = FALSE, error=FALSE,eval=FALSE, warning=FALSE}
SELECT *
FROM ORDERS
```
